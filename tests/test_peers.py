...
from stock_analyst.engine.peers import PeerAnalyzer, _strip_exchange
...
class TestDiscoverPeers:
    def test_returns_from_cache(self, null_cache, config):
        cache = MagicMock()
        cache.get.return_value = ["INFY", "WIPRO"]
        provider = MagicMock()
    ...
    def test_removes_target(self, peer_analyzer):
        peer_analyzer._yfinance_industry = MagicMock(return_value=["TCS", "INFY", "WIPRO"])
    ...
    def test_respects_max_count(self, config, null_cache):
        ...
        provider = MagicMock()
        pa = PeerAnalyzer(provider, null_cache, config)
        pa._yfinance_industry = MagicMock(return_value=["INFY", "WIPRO", "HCLTECH", "TECHM"])
        result = pa.discover_peers("TCS")
        assert len(result) <= 2


class TestGetPeerFundamentals:
    def test_fetches_all(self, peer_analyzer, sample_info):
        peer_analyzer._provider.get_info.return_value = sample_info
        result = peer_analyzer.get_peer_fundamentals("TCS", ["INFY", "WIPRO"])
        assert "TCS" in result
        assert "INFY" in result
        assert "WIPRO" in result
        assert result["TCS"]["pe"] == 30.0
...
class TestGetPeerTechnicals:
    def test_fetches_all(self, peer_analyzer, sample_ohlcv):
        peer_analyzer._provider.get_history.return_value = sample_ohlcv
        result = peer_analyzer.get_peer_technicals("TCS", ["INFY"])
        assert "TCS" in result
        assert "INFY" in result
